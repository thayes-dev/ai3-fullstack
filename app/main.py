...
Session 3.2: Wire feedback capture into the chat display.
...
# Load environment variables (.env has PHOENIX_API_KEY, ANTHROPIC_API_KEY, etc.)
from dotenv import load_dotenv
load_dotenv(_PROJECT_ROOT / ".env")

# ============================================================
# Phoenix Tracing Setup (Session 3.2)
# ============================================================
# Three pieces make traces appear in Phoenix:
#   1. register() — connects to Phoenix (where to send traces)
#      auto_instrument=True auto-discovers and instruments all
#      available libraries (Anthropic, etc.)
#   2. using_session() — groups multi-turn conversations together
#      as a "session" in Phoenix (see Step 5 below)
#   3. Parent span in rag.py — wraps the full pipeline so all
#      child spans (Claude call, etc.) nest under one trace
# ============================================================
...
# Unique session ID per browser session — groups all chat turns
# together as one "session" in Phoenix.
...
# ============================================================
# STEP 6: Feedback Handlers (Session 3.2)
# ============================================================
# These callbacks fire when a user clicks thumbs up/down or submits
# a text note. They save feedback to session state AND submit to
# Phoenix as span annotations for observability.

def _save_feedback(index: int):
...
def _save_feedback_note(index: int):
    """Callback: submit text note for negative feedback to Phoenix."""
...
def render_feedback(index: int):
    """Render the feedback widget for a message at the given index."""
    message = st.session_state.messages[index]
    existing_fb = message.get("feedback", None)

    # Pre-set widget state from stored feedback
    st.session_state[f"fb_{index}"] = existing_fb

    # Thumbs widget — disabled after selection (visual confirmation)
    st.feedback(
        "thumbs",
        key=f"fb_{index}",
    ...
    # Negative feedback: show text input for details
    if existing_fb == 0 and not message.get("feedback_note"):
        st.text_input(
            "What went wrong?",
            key=f"note_{index}",
            placeholder="Help us improve this response (press Enter to submit)",
...
# ============================================================
# Display all previous messages with feedback
# ============================================================
...
if prompt := st.chat_input("Ask a question..."):
    ...
    # Wrap in using_attributes so this turn groups with the rest
    # of this conversation as a session in Phoenix.
    try:
    ...
    # Append BEFORE rendering so the callback can find the message
    ...
    with st.chat_message("assistant"):
        ...
        # Feedback widget — uses same render_feedback as history loop
